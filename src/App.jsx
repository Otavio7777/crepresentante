...
function MobileApp({ data, user, onLogout }) {
  ...
  const LABELS = { home:'Início', pedidos:'Tirar Pedido', conversas:'Conversas', contatos:'Contatos', mais:'Mais' }
  const SIDE_NAV = [
    { section:'Principal' },
  ...
  return (
    <div style={{ fontFamily:"'Inter',system-ui,sans-serif", background:B[50], height:'100dvh', display:'flex', flexDirection:'column', overflow:'hidden', maxWidth:430, margin:'0 auto', position:'relative' }}>
      ...
      <div style={{ background:B[0], borderTop:`1px solid ${B[150]}`, display:'flex', flexShrink:0 }}>
        {[['home','Início','home'],['pedidos','Pedidos','cart'],['conversas','Chat','chat'],['contatos','Contatos','user'],['mais','Mais','bar']].map(([id,lbl,icon])=>(
...
function MobileConversas({ data }) {
  ...
  if (activeId) return <ChatPanel contact={ac} msgs={msgs[activeId]||[]} onSend={sendMsg} onBack={()=>setActiveId(null)} />
  return (
    <div>
      {data.contacts.map(c=>{
...
function DesktopApp({ data, user, onLogout }) {
  ...
  const NAV = [
    ...
    { id:'erp',       label:'ERP / Integração', icon:'erp' },
  ]

  const LABELS = { dashboard:'Dashboard', pedidos:'Pedidos', conversas:'Conversas', contatos:'Carteira de Contatos', funil:'Funil de Vendas', metas:'Metas & Performance', gestao:'Gestão & Relatórios', erp:'Integração ERP' }
  ...
  return (
    <div style={{ fontFamily:"'Inter',system-ui,sans-serif", background:B[50], height:'100dvh', display:'flex', overflow:'hidden' }}>
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden', minWidth:0 }}>
        ...
        {/* Content */}
        <div style={{ flex:1, overflow:['conversas','pedidos'].includes(tab)?'hidden':'auto' }}>
